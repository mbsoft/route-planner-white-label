...
export default function RouteAnalysisPage() {
  ...
  const fetchOptimizationResults = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setError(t('failedToFetchOptimizationResults'))
      }
    } catch (error) {
  ...
  const handleSaveTitle = async (result: any) => {
    try {
      const response = await fetch(`/api/optimization-results/${result.id}`, {
        method: 'PATCH',
        headers: {
      ...
      if (response.ok) {
        ...
        console.log(t('titleUpdatedSuccessfully'))
      } else {
        console.error(t('failedToUpdateTitle'))
      }
    } catch (error) {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      const response = await fetch(`/api/optimization-results/${resultToDelete.id}`, {
        method: 'DELETE',
      })
      ...
      if (response.ok) {
        ...
        // Close the selected result if it was the one being deleted
        ...
        console.log(t('resultDeletedSuccessfully'))
      } else {
        console.error(t('failedToDeleteResult'))
      }
    } catch (error) {
              ...
              <Grid item xs={12} md={6} lg={3}>
                <Card sx={{ height: '100%', p: 3 }}>
                  <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
                    ...
                    <Typography variant="body1" sx={{ color: '#666', fontWeight: 'bold' }}>
                      {tMetrics('averageGallonsPerRoute')}
                    </Typography>
                  </Box>
                  <Box sx={{ display: 'flex', alignItems: 'baseline', gap: 1 }}>
              ...
              <Grid item xs={12} md={6} lg={3}>
                <Card sx={{ height: '100%', p: 3 }}>
                  <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
                    ...
                    <Typography variant="body1" sx={{ color: '#666', fontWeight: 'bold' }}>
                      {tMetrics('averageDistancePerRoute')}
                    </Typography>
                  </Box>
                  <Box sx={{ display: 'flex', alignItems: 'baseline', gap: 1 }}>
              ...
              <Grid item xs={12} md={6} lg={3}>
                <Card sx={{ height: '100%', p: 3 }}>
                  <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
                    ...
                    <Typography variant="body1" sx={{ color: '#666', fontWeight: 'bold' }}>
                      {tMetrics('averageServiceTimePerRoute')}
                    </Typography>
                  </Box>
                  <Box sx={{ display: 'flex', alignItems: 'baseline', gap: 1 }}>
                          ...
                          <Table>
                            <TableHead>
                              <TableRow>
                                ...
                                <TableCell sx={{ fontWeight: 'bold' }}>{t('solutionTime')}</TableCell>
                                    ...
                                    {editingTitle === result.id ? (
                                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                                        <input
                                          ...
                                          aria-label={t('editOptimizationTitle')}
                                          title={t('editOptimizationTitle')}
                                        />
                                        <Button
                                          size="small"
                        ...
                        >
                          ...
                          <Grid container spacing={2} sx={{ mb: 3 }}>
                            ...
                            <Grid item xs={12} md={6}>
                              ...
                              <Typography variant="body2" sx={{ mb: 2 }}>
                                {selectedResult.solution_time ? `${selectedResult.solution_time.toFixed(2)} ${t('seconds')}` : 'N/A'}
                              </Typography>
                            </Grid>
                          </Grid>
                          ...
                          {selectedResult?.response_data?.result?.routes && (
                            <Box sx={{ mt: 3 }}>
                              ...
                              <RouteSummaryTable
                                ...
                                onToggleRoute={(index: number) => {
                                  setExpandedRoutes((prev: Set<number>) => {
          ...
          <Box sx={{
            ...
            <Box sx={{ mt: 0, pt: 0, display: 'flex', alignItems: 'center', gap: 1 }}>
              ...
              <Typography variant="body2" sx={{ color: '#999', fontSize: '14px' }}>
                {tFooter('poweredBy')} | {tFooter('version')} | {tFooter('lastUpdated')}: {new Date().toLocaleDateString()}
              </Typography>
            </Box>
          </Box>
        </Box>
      ...
      >
        ...
        <DialogContent>
          <Typography id="delete-dialog-description">
            {t('deleteConfirmationMessage', { title: resultToDelete?.title })}
          </Typography>
